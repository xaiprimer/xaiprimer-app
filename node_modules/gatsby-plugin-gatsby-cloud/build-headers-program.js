...
const writeHeadersFile = ({
  publicFolder
}) => contents => (0, _fsExtra.writeFile)(publicFolder(_constants.HEADERS_FILENAME), JSON.stringify(contents));

function buildHeadersProgram(pluginData, pluginOptions, reporter) {
  return _lodash.default.flow(mapUserLinkHeaders(pluginData), applySecurityHeaders(pluginOptions), applyCachingHeaders(pluginData, pluginOptions), mapUserLinkAllPageHeaders(pluginData, pluginOptions), applyLinkHeaders(pluginData, pluginOptions), applyTransfromHeaders(pluginOptions), writeHeadersFile(pluginData))(pluginOptions.headers);
}